...
%% Load Wet Dry Line
F=fopen('C:\CoastSnap\CoastSnap_NonGuiWorkflow-master\CoastSnap_NonGuiWorkflow-master\X_Analysis\wetdrylinelook\20210702_jennettes_low_water_7am.txt');
...
I=imread('C:\CoastSnap\CoastSnap_NonGuiWorkflow-master\CoastSnap_NonGuiWorkflow-master\X_JennettesExampleData\ObliqueImagery\1625227200.Fri.Jul.02_07_00_00.EST.2021.jennettes.snap.anonymous.jpg');
f1=figure;
...
% Load IOEO
load('C:\CoastSnap\CoastSnap_NonGuiWorkflow-master\CoastSnap_NonGuiWorkflow-master\X_JennettesExampleData\IOEO_walked_shorelines\20210702_IOEO')
...
xyzCheck = [wd.e wd.n  wd.z.*0+0.6599855];  % N x 3 matrix with rows= N pts, columns= x,y,z
% Transform xyz World Coordinates to Distorted Image Coordinates
[UVdReproj ] = xyz2DistUV(IOEO,xyzCheck);
%  Reshape UVdCheck so easier to interpret
UVdReproj = reshape(UVdReproj ,[],2);
...
xyzCheck = [wd.e wd.n  wd.z.*0+(-0.5)];  % N x 3 matrix with rows= N pts, columns= x,y,z
% Transform xyz World Coordinates to Distorted Image Coordinates
[UVdReproj ] = xyz2DistUV(IOEO,xyzCheck);
%  Reshape UVdCheck so easier to interpret
UVdReproj = reshape(UVdReproj ,[],2);
...
xyzCheck = [wd.e wd.n  wd.z.*0+(-0.498)];  % N x 3 matrix with rows= N pts, columns= x,y,z
% Transform xyz World Coordinates to Distorted Image Coordinates
[UVdReproj ] = xyz2DistUV(IOEO,xyzCheck);
%  Reshape UVdCheck so easier to interpret
UVdReproj = reshape(UVdReproj ,[],2);
...
xyzCheck = [wd.e wd.n  wd.z.*0+.228];  % N x 3 matrix with rows= N pts, columns= x,y,z
% Transform xyz World Coordinates to Distorted Image Coordinates
[UVdReproj ] = xyz2DistUV(IOEO,xyzCheck);
%  Reshape UVdCheck so easier to interpret
UVdReproj = reshape(UVdReproj ,[],2);
