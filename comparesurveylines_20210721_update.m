...
%% Load Wet Dry Line
F=fopen('C:\CoastSnap\CoastSnap_NonGuiWorkflow-master\CoastSnap_NonGuiWorkflow-master\X_Analysis\wetdrylinelook\20210721_jennettes_high_tide.txt');
...
I=imread('C:\CoastSnap\CoastSnap_NonGuiWorkflow-master\CoastSnap_NonGuiWorkflow-master\X_JennettesExampleData\ObliqueImagery\1626866940.Wed.Jul.21_06_29_00.EST.2021.jennettes.snap.anonymous.jpg');
f1=figure;
...
% Load IOEO
load('C:\CoastSnap\CoastSnap_NonGuiWorkflow-master\CoastSnap_NonGuiWorkflow-master\X_JennettesExampleData\IOEO_walked_shorelines\20210721_IOEO')
...
xyzCheck = [wd.e wd.n  wd.z.*0+0.259680539];  % N x 3 matrix with rows= N pts, columns= x,y,z
% Transform xyz World Coordinates to Distorted Image Coordinates
[UVdReproj ] = xyz2DistUV(IOEO,xyzCheck);
%  Reshape UVdCheck so easier to interpret
UVdReproj = reshape(UVdReproj ,[],2);
...
xyzCheck = [wd.e wd.n  wd.z.*0+0.16];  % N x 3 matrix with rows= N pts, columns= x,y,z
% Transform xyz World Coordinates to Distorted Image Coordinates
[UVdReproj ] = xyz2DistUV(IOEO,xyzCheck);
%  Reshape UVdCheck so easier to interpret
UVdReproj = reshape(UVdReproj ,[],2);
...
xyzCheck = [wd.e wd.n  wd.z.*0+.8777];  % N x 3 matrix with rows= N pts, columns= x,y,z
% Transform xyz World Coordinates to Distorted Image Coordinates
[UVdReproj ] = xyz2DistUV(IOEO,xyzCheck);
%  Reshape UVdCheck so easier to interpret
UVdReproj = reshape(UVdReproj ,[],2);
